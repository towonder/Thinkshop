...
{ 
    ...
    for ($i = 0  ; $i <= $last_index  ; $i++) { 
        ...
        if ($i < $last_index) { // no need to build link for last item 
            $link = sprintf('<span class="crumb"><a href="%s" class="%s">%s</a></span>', $link, $this->link_class, $title, $this->separator)        ; 
        } else { 
            $link = sprintf("<span class='%s'><p>%s</p></span>", $this->span_class, $title )                    ;     //last text, ie current page without link 
        } 
        $string    .=  $link                            ; 
    } 
    return $string; 
